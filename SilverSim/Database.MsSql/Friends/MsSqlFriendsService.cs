...
namespace SilverSim.Database.MsSql.Friends
{
    public static class MySQLFriendsExtensionMethods
    {
        public static FriendInfo ToFriendInfo(this SqlDataReader reader)
        {
    ...
    [Description("MsSql Friends Backend")]
    [PluginName("Friends")]
    public sealed class MsSqlFriendsService : FriendsServiceInterface, IPlugin, IDBServiceInterface, IUserAccountDeleteServiceInterface
    {
        private readonly string m_ConnectionString;
        private static readonly ILog m_Log = LogManager.GetLogger("MSSQL FRIENDS SERVICE");
        ...
        public override List<FriendInfo> this[UGUI user]
        {
            get
            {
                ...
                {
                    connection.Open();
                    using (var cmd = new SqlCommand(m_InnerJoinSelectFull + "WHERE A.UserID = @id", connection))
                    {
        ...
        public override void Delete(FriendInfo fi)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("DELETE FROM friends WHERE (UserID = @userid AND FriendID = @friendid) OR (UserID = @friendid AND FriendID = @userid)", connection))
                {
        ...
        public void Remove(UUID scopeID, UUID accountID)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("DELETE FROM friends WHERE UserID = @id OR FriendID = @id", connection))
                {
        ...
        public override void StoreOffer(FriendInfo fi)
        {
            using (var connection = new SqlConnection(m_ConnectionString))
            {
                ...
                connection.ReplaceInto("friends", vals, new string[] { "UserID", "FriendID" });
            }
        }
        ...
        public override void StoreRights(FriendInfo fi)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("UPDATE friends SET RightsToFriend = @rights WHERE UserID = @userid AND FriendID = @friendid", connection))
                {
        ...
        public override bool TryGetValue(UGUI user, UGUI friend, out FriendInfo fInfo)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand(m_InnerJoinSelectFull + "WHERE A.UserID = @userid AND A.FriendID = @friendid", connection))
                {
