...
namespace SilverSim.Database.MsSql.OfflineIM
{
    [Description("MsSql OfflineIM Backend")]
    [PluginName("OfflineIM")]
    public sealed class MsSqlOfflineIMService : OfflineIMServiceInterface, IPlugin, IDBServiceInterface, IUserAccountDeleteServiceInterface
    {
        private static readonly ILog m_Log = LogManager.GetLogger("MSSQL OFFLINEIM SERVICE");
        ...
        public override void DeleteOfflineIM(ulong offlineImID)
        {
            ...
            {
                connection.Open();
                using (var command = new SqlCommand("DELETE FROM offlineim WHERE ID = @id", connection))
                {
        ...
        public override List<GridInstantMessage> GetOfflineIMs(UUID principalID)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("SELECT * FROM offlineim WHERE ToAgentID = @id", connection))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            var im = new GridInstantMessage
                            {
                                ...
                                ToAgent = new UGUI(reader.GetUUID("ToAgentID")),
        ...
        public void Remove(UUID accountID)
        {
            ...
            {
                connection.Open();
                using (var command = new SqlCommand("DELETE FROM offlineim WHERE ToAgentID = @id", connection))
                {
