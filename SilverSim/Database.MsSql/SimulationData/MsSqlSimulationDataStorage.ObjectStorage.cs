...
{
    public sealed partial class MsSqlSimulationDataStorage : ISimulationDataObjectStorageInterface
    {
        ...
        List<UUID> ISimulationDataObjectStorageInterface.ObjectsInRegion(UUID key)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("SELECT [ID] FROM objects WHERE [RegionID] = @regionid", connection))
                {
        ...
        List<UUID> ISimulationDataObjectStorageInterface.PrimitivesInRegion(UUID key)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("SELECT [ID] FROM prims WHERE [RegionID] = @regionid", connection))
                {
                    cmd.Parameters.AddParameter("@regionid", key);
                    using (SqlDataReader dbReader = cmd.ExecuteReader())
                    {
        ...
        #region helpers
        private ObjectGroup ObjectGroupFromDbReader(SqlDataReader dbReader) => new ObjectGroup
        {
        ...
        private ObjectPart ObjectPartFromDbReader(SqlDataReader dbReader)
        {
        ...
        private ObjectPartInventoryItem ObjectPartInventoryItemFromDbReader(SqlDataReader dbReader)
        {
        ...
        List<ObjectGroup> ISimulationDataObjectStorageInterface.this[UUID regionID]
        {
            get
            {
                ...
                using (var connection = new SqlConnection(m_ConnectionString))
                {
                    ...
                    using (var cmd = new SqlCommand("SELECT * FROM objects WHERE RegionID = @regionid", connection))
                    {
                    ...
                    int primcount = 0;
                    using (var cmd = new SqlCommand("SELECT * FROM prims WHERE RegionID = @regionid", connection))
                    {
                        cmd.Parameters.AddParameter("@regionid", regionID);
                        cmd.CommandTimeout = 3600;
                        using (SqlDataReader dbReader = cmd.ExecuteReader())
                    ...
                    using (var cmd = new SqlCommand("SELECT * FROM primitems WHERE RegionID = @regionid", connection))
                    {
                        cmd.Parameters.AddParameter("@regionid", regionID);
                        cmd.CommandTimeout = 3600;
                        using (SqlDataReader dbReader = cmd.ExecuteReader())
