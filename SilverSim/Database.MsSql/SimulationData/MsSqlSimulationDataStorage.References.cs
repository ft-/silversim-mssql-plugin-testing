...
{
    public sealed partial class MsSqlSimulationDataStorage : IAssetReferenceInfoServiceInterface
    {
        public void EnumerateUsedAssets(Action<UUID> action)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT DISTINCT AssetId FROM primitems", conn))
                {
                ...
                using (var cmd = new SqlCommand("SELECT EnvironmentSettings FROM environmentsettings", conn))
                {
                    using (SqlDataReader dbReader = cmd.ExecuteReader())
                    {
                        while (dbReader.Read())
                        {
                            ...
                            using (var ms = new MemoryStream(dbReader.GetBytes("EnvironmentSettings")))
                            {
                ...
                using (var cmd = new SqlCommand("SELECT DISTINCT NormalMapTexture FROM lightshare", conn))
                {
                    using (SqlDataReader dbReader = cmd.ExecuteReader())
                    {
                        while (dbReader.Read())
                ...
                using (var cmd = new SqlCommand("SELECT DISTINCT TerrainTexture1, TerrainTexture2, TerrainTexture3, TerrainTexture4 FROM regionsettings", conn))
                {
                    using (SqlDataReader dbReader = cmd.ExecuteReader())
                    {
                        while (dbReader.Read())
                ...
                using (var cmd = new SqlCommand("SELECT PrimitiveShapeData, ParticleSystem, TextureEntryBytes, ProjectionData, LoopedSoundData, ImpactSoundData FROM prims", conn))
                {
                    using (SqlDataReader dbReader = cmd.ExecuteReader())
                    {
                        while (dbReader.Read())
