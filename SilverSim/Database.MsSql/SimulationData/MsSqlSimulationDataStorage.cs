...
{
    [Description("Ms Sql Simulation Data Backend")]
    [PluginName("SimulationData")]
    public sealed partial class MsSqlSimulationDataStorage : SimulationDataStorageInterface, IDBServiceInterface, IPlugin, IQueueStatsAccess
    {
        private static readonly ILog m_Log = LogManager.GetLogger("MSSQL SIMULATION STORAGE");
        ...
        public override void RemoveRegion(UUID regionID)
        {
            ...
            foreach (string table in Tables)
            {
                ...
                {
                    connection.Open();
                    using (var cmd = new SqlCommand("DELETE FROM " + table + " WHERE RegionID = '" + regionIdStr + "'", connection))
                    {
                        cmd.ExecuteNonQuery();
                    }
                }
        ...
        IList<QueueStatAccessor> IQueueStatsAccess.QueueStats
        {
            get
            {
                ...
                foreach (MsSqlTerrainListener terListener in m_TerrainListenerThreads)
                {
                    statFuncs.Add(new QueueStatAccessor("TerrainStore." + terListener.RegionID.ToString(), terListener.GetStats));
                }

                foreach (MsSqlSceneListener sceneListener in m_SceneListenerThreads)
                {
