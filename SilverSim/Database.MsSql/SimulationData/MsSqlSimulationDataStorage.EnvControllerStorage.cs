...
{
    public sealed partial class MsSqlSimulationDataStorage : ISimulationDataEnvControllerStorageInterface
    {
        bool ISimulationDataEnvControllerStorageInterface.TryGetValue(UUID regionID, out byte[] settings)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT TOP(1) SerializedData FROM environmentcontroller WHERE RegionID = @regionid", conn))
                {
            ...
            set
            {
                using (var conn = new SqlConnection(m_ConnectionString))
                {
...
#endif
                        using (var cmd = new SqlCommand("DELETE FROM environmentcontroller WHERE RegionID = @regionid", conn))
                        {
                            cmd.Parameters.AddParameter("@regionid", regionID);
                            cmd.ExecuteNonQuery();
                        }
                    ...
                    else
                    {
                        ...
                        conn.ReplaceInto("environmentcontroller", param, new string[] { "RegionID" });
                    }
                }
            }
        }
