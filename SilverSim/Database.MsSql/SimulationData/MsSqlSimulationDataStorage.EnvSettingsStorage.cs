...
{
    public sealed partial class MsSqlSimulationDataStorage : ISimulationDataEnvSettingsStorageInterface
    {
            ...
            set
            {
                using (var conn = new SqlConnection(m_ConnectionString))
                {
...
#endif
                        using (var cmd = new SqlCommand("DELETE FROM environmentsettings WHERE RegionID = @regionid", conn))
                        {
                            cmd.Parameters.AddParameter("@regionid", regionID);
                            cmd.ExecuteNonQuery();
                        }
                    ...
                    else
                    {
                        ...
                        conn.ReplaceInto("environmentsettings", param, new string[] { "RegionID" });
                    }
                }
            }
        }
