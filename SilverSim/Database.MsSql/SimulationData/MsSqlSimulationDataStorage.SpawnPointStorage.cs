...
{
    public sealed partial class MsSqlSimulationDataStorage : ISimulationDataSpawnPointStorageInterface
    {
        List<Vector3> ISimulationDataSpawnPointStorageInterface.this[UUID regionID]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new SqlCommand("SELECT DistanceX, DistanceY, DistanceZ FROM spawnpoints WHERE RegionID = @regionid", conn))
                    {
            ...
            set
            {
                using (var conn = new SqlConnection(m_ConnectionString))
                {
                    ...
                    {
                        using (var cmd = new SqlCommand("DELETE FROM spawnpoints WHERE RegionID = @regionid", conn))
                        {
