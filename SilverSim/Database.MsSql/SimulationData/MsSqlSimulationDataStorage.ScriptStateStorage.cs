...
{
    public sealed partial class MsSqlSimulationDataStorage : ISimulationDataScriptStateStorageInterface
    {
        ...
        byte[] ISimulationDataScriptStateStorageInterface.this[UUID regionID, UUID primID, UUID itemID]
        {
            ...
            set
            {
                using (var connection = new SqlConnection(m_ConnectionString))
                {
                    ...
                    connection.ReplaceInto("scriptstates", p, new string[] { "RegionID", "PrimID", "ItemID" });
                }
            }
        }
        ...
        bool ISimulationDataScriptStateStorageInterface.Remove(UUID regionID, UUID primID, UUID itemID)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("DELETE FROM scriptstates WHERE RegionID = @regionid AND PrimID = @primid AND ItemID = @itemid", connection))
                {
                    cmd.Parameters.AddParameter("@regionid", regionID);
                    cmd.Parameters.AddParameter("@primid", primID);
                    cmd.Parameters.AddParameter("@itemid", itemID);
