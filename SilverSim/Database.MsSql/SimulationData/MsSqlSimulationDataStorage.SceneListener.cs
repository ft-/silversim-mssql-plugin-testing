...
namespace SilverSim.Database.MsSql.SimulationData
{
    ...
    {
        private readonly RwLockedList<MsSqlSceneListener> m_SceneListenerThreads = new RwLockedList<MsSqlSceneListener>();
        public class MsSqlSceneListener : SceneListener
        {
            private readonly string m_ConnectionString;
            private readonly RwLockedList<MsSqlSceneListener> m_SceneListenerThreads;

            public MsSqlSceneListener(string connectionString, UUID regionID, RwLockedList<MsSqlSceneListener> sceneListenerThreads)
                : base(regionID)
            {
            ...
            private void ProcessPrimItemDeletions(SqlConnection conn)
            {
            ...
            private void ProcessPrimDeletions(SqlConnection conn)
            {
            ...
            private void ProcessGroupDeletions(SqlConnection conn)
            {
                var removedItems = new List<UUID>();
            ...
            private void ProcessPrimItemUpdates(SqlConnection conn)
            {
            ...
            private void ProcessPrimUpdates(SqlConnection conn)
            {
                foreach (UUID k in m_PrimUpdates.Keys.ToArray())
                {
                    conn.ReplaceInto("prims", m_PrimUpdates[k], m_PrimKeys);
            ...
            private void ProcessGroupUpdates(SqlConnection conn)
            {
                foreach (UUID k in m_GroupUpdates.Keys.ToArray())
                {
                    conn.ReplaceInto("objects", m_GroupUpdates[k], m_ObjectKeys);
                }
            }
        ...
        public override SceneListener GetSceneListener(UUID regionID) =>
            new MsSqlSceneListener(m_ConnectionString, regionID, m_SceneListenerThreads);
    }
}
