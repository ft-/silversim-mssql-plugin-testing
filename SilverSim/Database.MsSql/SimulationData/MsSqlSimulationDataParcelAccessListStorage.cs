...
{
    public sealed class MsSqlSimulationDataParcelAccessListStorage : ISimulationDataParcelAccessListStorageInterface
    {
        private readonly string m_ConnectionString;
        private readonly string m_TableName;

        public MsSqlSimulationDataParcelAccessListStorage(string connectionString, string tableName)
        {
        ...
        public bool TryGetValue(UUID regionID, UUID parcelID, UUI accessor, out ParcelAccessEntry e)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("DELETE FROM " + m_TableName + " WHERE [ExpiresAt] <= " + Date.GetUnixTime().ToString() + " AND [ExpiresAt] <> 0", connection))
                {
                    cmd.ExecuteNonQuery();
                }

                /* we use a specific implementation to reduce the result set here */
                using (var cmd = new SqlCommand("SELECT * FROM " + m_TableName + " WHERE [RegionID] = '" + regionID.ToString() + "' AND [ParcelID] = '" + parcelID.ToString() + "' AND [Accessor] LIKE '" + accessor.ID.ToString() + "%'", connection))
                {
        ...
        public List<ParcelAccessEntry> this[UUID regionID, UUID parcelID]
        {
            get
            {
                ...
                using (var connection = new SqlConnection(m_ConnectionString))
                {
                    ...
                    using (var cmd = new SqlCommand("SELECT * FROM " + m_TableName + " WHERE [RegionID] = '" + regionID.ToString() + "' AND [ParcelID] = '" + parcelID.ToString() + "'", connection))
                    {
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
        ...
        public void Store(ParcelAccessEntry entry)
        {
            using (var connection = new SqlConnection(m_ConnectionString))
            {
                ...
                connection.ReplaceInto(m_TableName, data, new string[] { "RegionID", "ParcelID", "Accessor" });
            }
        }
            ...
            using (var connection = new SqlConnection(m_ConnectionString))
            {
                ...
                connection.InsideTransaction((transaction) =>
                {
                    ...
                    })
                    {
                        ...
                        cmd.Parameters.AddParameter("@ExpiresAt", Date.Now.AsULong);
                    ...
                    using (var cmd = new SqlCommand("UPDATE " + m_TableName + " SET [ExpiresAt] = [ExpiresAt] + @extendseconds WHERE [RegionID] = @RegionID AND [ParcelID] = @ParcelID AND [Accessor] = @Accessor AND [ExpiresAt] > 0", connection)
                    {
                        Transaction = transaction
                    })
                    {
        ...
        public bool RemoveAllFromRegion(UUID regionID)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("DELETE FROM " + m_TableName + " WHERE [RegionID] = '" + regionID.ToString() + "'", connection))
                {
        ...
        public bool Remove(UUID regionID, UUID parcelID)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("DELETE FROM " + m_TableName + " WHERE [RegionID] = '" + regionID.ToString() + "' AND [ParcelID] = '" + parcelID.ToString() + "'", connection))
                {
                    return cmd.ExecuteNonQuery() > 0;
                }
            }
