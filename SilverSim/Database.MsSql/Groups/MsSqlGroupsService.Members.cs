...
{
    public sealed partial class MsSqlGroupsService : GroupsServiceInterface.IGroupMembersInterface
    {
        ...
        bool IGroupMembersInterface.ContainsKey(UUI requestingAgent, UGI group, UUI principal)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT GroupID FROM groupmemberships WHERE GroupID = @groupid AND PrincipalID = @principalid", conn))
                {
                    cmd.Parameters.AddParameter("@groupid", group.ID);
                    cmd.Parameters.AddParameter("@principalid", principal.ID);
                    using (SqlDataReader reader = cmd.ExecuteReader())
        ...
        bool IGroupMembersInterface.TryGetValue(UUI requestingAgent, UGI group, UUI principal, out GroupMember gmem)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT * FROM groupmemberships WHERE GroupID = @groupid AND PrincipalID = @principalid", conn))
                {
                    cmd.Parameters.AddParameter("@groupid", group.ID);
                    cmd.Parameters.AddParameter("@principalid", principal.ID);
                    using (SqlDataReader reader = cmd.ExecuteReader())
