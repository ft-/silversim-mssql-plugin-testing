...
{
    [Description("MsSql Groups Backend")]
    [PluginName("Groups")]
    public sealed partial class MsSqlGroupsService : GroupsServiceInterface, IPlugin, IDBServiceInterface, IUserAccountDeleteServiceInterface
    {
        private static readonly ILog m_Log = LogManager.GetLogger("MSSQL GROUPS SERVICE");
        ...
        public void Remove(UUID scopeID, UUID accountID)
        {
            using (var conn = new SqlConnection(m_ConnectionString))
            {
                ...
                {
                    using (var cmd = new SqlCommand("DELETE FROM groupinvites WHERE PrincipalID = @id", conn))
                    {
                        cmd.Transaction = transaction;
                        cmd.Parameters.AddParameter("@id", accountID);
                    }
                    using (var cmd = new SqlCommand("DELETE FROM groupmemberships WHERE PrincipalID = @id", conn))
                    {
                        cmd.Transaction = transaction;
                        cmd.Parameters.AddParameter("@id", accountID);
                    }
                    using (var cmd = new SqlCommand("DELETE FROM activegroup WHERE PrincipalID = @id", conn))
                    {
                        cmd.Transaction = transaction;
                        cmd.Parameters.AddParameter("@id", accountID);
                    }
                    using (var cmd = new SqlCommand("DELETE FROM grouprolememberships WHERE PrincipalID = @id", conn))
                    {
                        cmd.Transaction = transaction;
                        cmd.Parameters.AddParameter("@id", accountID);
                    }
