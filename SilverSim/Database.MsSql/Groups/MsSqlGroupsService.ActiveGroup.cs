    ...
    public sealed partial class MsSqlGroupsService : GroupsServiceInterface.IGroupSelectInterface
    {
        UGI IGroupSelectInterface.this[UGUI requestingAgent, UGUI principalID]
        {
            ...
            set
            {
                if (Members.ContainsKey(requestingAgent, value, principalID))
                {
                    ...
                    {
                        conn.Open();
                        using (var cmd = new SqlCommand("INSERT INTO activegroup (PrincipalID, ActiveGroupID) VALUES (@principalid, @activegroupid) ON DUPLICATE KEY UPDATE ActiveGroupID=@activegroupid", conn))
                        {
                            cmd.Parameters.AddParameter("@principalid", principalID.ID);
        ...
        UUID IGroupSelectInterface.this[UGUI requestingAgent, UGI group, UGUI principal]
        {
            ...
            set
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new SqlCommand("UPDATE groupmemberships SET SelectedRoleID=@roleid WHERE PrincipalID = @principalid AND GroupID = @groupid", conn))
                    {
