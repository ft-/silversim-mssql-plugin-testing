...
{
    public sealed partial class MsSqlGroupsService : GroupsServiceInterface.IGroupsInterface
    {
        ...
        void IGroupsInterface.Delete(UUI requestingAgent, UGI group)
        {
            ...
            using (var conn = new SqlConnection(m_ConnectionString))
            {
                ...
                {
                    using (var cmd = new SqlCommand("DELETE FROM activegroup WHERE ActiveGroupID = @groupid", conn))
                    {
                    ...
                    {
                        using (var cmd = new SqlCommand("DELETE FROM " + table + " WHERE GroupID = @groupid", conn))
                        {
                            cmd.Transaction = transaction;
                            cmd.Parameters.AddParameter("@groupid", group.ID);
                            cmd.ExecuteNonQuery();
        ...
        List<DirGroupInfo> IGroupsInterface.GetGroupsByName(UUI requestingAgent, string query)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT g.GroupID, g.`Name`, g.Location, " + GCountQuery + " FROM groups AS g WHERE g.Name LIKE @value", conn))
                {
