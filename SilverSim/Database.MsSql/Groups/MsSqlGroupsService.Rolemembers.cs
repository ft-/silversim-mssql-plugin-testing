...
{
    public sealed partial class MsSqlGroupsService : GroupsServiceInterface.IGroupRolemembersInterface
    {
        List<GroupRolemember> IGroupRolemembersInterface.this[UGUI requestingAgent, UGI group]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new SqlCommand("SELECT rm.*, r.Powers FROM grouprolememberships AS rm INNER JOIN grouproles AS r ON rm.GroupID = r.GroupID AND rm.RoleID = r.RoleID WHERE rm.GroupID = @groupid", conn))
                    {
                    ...
                    using (var cmd = new SqlCommand("SELECT * FROM groupmemberships WHERE rm.PrincipalID = @principalid", conn))
                    {
                        cmd.Parameters.AddParameter("@groupid", group.ID);
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
        ...
        List<GroupRolemembership> IGroupRolemembersInterface.this[UGUI requestingAgent, UGUI principal]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new SqlCommand("SELECT rm.*, r.Powers, r.Title FROM grouprolememberships AS rm INNER JOIN grouproles AS r ON rm.GroupID = r.GroupID AND rm.RoleID = r.RoleID WHERE rm.PrincipalID = @principalid", conn))
                    {
        ...
        List<GroupRolemember> IGroupRolemembersInterface.this[UGUI requestingAgent, UGI group, UUID roleID]
        {
            get
            {
                ...
                if (UUID.Zero == roleID)
                {
                    ...
                    {
                        conn.Open();
                        using (var cmd = new SqlCommand("SELECT * FROM groupmemberships WHERE rm.GroupID = @groupid", conn))
                        {
                            cmd.Parameters.AddParameter("@groupid", group.ID);
                            using (SqlDataReader reader = cmd.ExecuteReader())
                            {
                ...
                else
                {
                    ...
                    {
                        conn.Open();
                        using (var cmd = new SqlCommand("SELECT rm.*, r.Powers FROM grouprolememberships AS rm INNER JOIN grouproles AS r ON rm.GroupID = r.GroupID AND rm.RoleID = r.RoleID WHERE rm.GroupID = @groupid AND rm.RoleID = @roleid", conn))
                        {
                            cmd.Parameters.AddParameter("@groupid", group.ID);
        ...
        void IGroupRolemembersInterface.Delete(UGUI requestingAgent, UGI group, UUID roleID, UGUI principal)
        {
            ...
            else
            {
                ...
                using (var conn = new SqlConnection(m_ConnectionString))
                {
                    ...
                    {
                        using (var cmd = new SqlCommand("UPDATE groupmemberships SET SelectedRoleID=@zeroid WHERE SelectedRoleID = @roleid AND GroupID = @groupid AND PrincipalID = @principalid", conn))
                        {
                        ...
                        {
                            using (var cmd = new SqlCommand("DELETE FROM " + table + " WHERE GroupID = @groupid AND RoleID = @roleid AND PrincipalID = @principalid", conn))
                            {
                                cmd.Transaction = transaction;
                                cmd.Parameters.AddParameter("@principalid", principal.ID);
                                cmd.Parameters.AddParameter("@groupid", group.ID);
