...
namespace SilverSim.Database.MsSql.Maptile
{
    ...
    public sealed class MsSqlMaptileService : MaptileServiceInterface, IPlugin, IDBServiceInterface
    {
        ...
        public override bool TryGetValue(UUID scopeid, GridVector location, int zoomlevel, out MaptileData data)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("SELECT * FROM maptiles WHERE LocX = @locx AND LocY = @locy AND ZoomLevel = @zoomlevel", connection))
                {
        ...
        public override void Store(MaptileData data)
        {
            using (var connection = new SqlConnection(m_ConnectionString))
            {
                ...
                connection.ReplaceInto("maptiles", vals, new string[] { "LocX", "LocY", "ZoomLevel", "ScopeID" });
            }
        }
