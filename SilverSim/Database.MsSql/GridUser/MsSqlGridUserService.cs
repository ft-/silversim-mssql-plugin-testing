...
namespace SilverSim.Database.MsSql.GridUser
{
    [Description("MsSql GridUser Backend")]
    [PluginName("GridUser")]
    public sealed class MsSqlGridUserService : GridUserServiceInterface, IDBServiceInterface, IPlugin, IUserAccountDeleteServiceInterface
    {
        private readonly string m_ConnectionString;
        private static readonly ILog m_Log = LogManager.GetLogger("MSSQL GRIDUSER SERVICE");

        #region Constructor
        public MsSqlGridUserService(IConfig ownSection)
        {
        ...
        public override void LoggedInAdd(UGUI userID)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("UPDATE griduser SET IsOnline = 1, LastLogin = @curtime WHERE ID = @id", conn))
                {
                ...
                conn.ReplaceInto("griduser", param, new string[] { "ID" });
            }
        }
        ...
        public void Remove(UUID scopeID, UUID userAccount)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("DELETE FROM griduser WHERE ID = @id", conn))
                {
