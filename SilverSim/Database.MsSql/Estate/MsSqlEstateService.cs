...
namespace SilverSim.Database.MsSql.Estate
{
    ...
    [PluginName("Estate")]
    public sealed partial class MsSqlEstateService : EstateServiceInterface, IDBServiceInterface, IPlugin
    {
        private readonly string m_ConnectionString;
        private static readonly ILog m_Log = LogManager.GetLogger("MSSQL ESTATE SERVICE");

        #region Constructor
        public MsSqlEstateService(IConfig ownSection)
        {
        ...
        private static readonly IMigrationElement[] Migrations = new IMigrationElement[]
        {
            ...
            new NamedKeyInfo("Owner", "OwnerID"),
            new NamedKeyInfo("ID_Owner", "ID", "OwnerID"),
            new TableRevision(2),
        ...
        public override void Update(EstateInfo value)
        {
            ...
            {
                conn.Open();
                conn.ReplaceInto("estates", dict, new string[] { "ID" });
            }
        }
        ...
        public override bool Remove(uint estateID)
        {
            using (var conn = new SqlConnection(m_ConnectionString))
            {
                ...
                {
                    using (var cmd = new SqlCommand("DELETE FROM " + table + " WHERE EstateID = @id", conn))
                    {
                        cmd.Parameters.AddParameter("@id", estateID);
                        cmd.ExecuteNonQuery();
                    }
                }
                using (var cmd = new SqlCommand("DELETE FROM estates WHERE ID = @id", conn))
                {
                    cmd.Parameters.AddParameter("@id", estateID);
        ...
        public override List<EstateInfo> All
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new SqlCommand("SELECT * FROM estates", conn))
                    {
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
        ...
        public override List<uint> AllIDs
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new SqlCommand("SELECT ID FROM estates", conn))
                    {
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
