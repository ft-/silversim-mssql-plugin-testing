...
{
    public sealed partial class MsSqlEstateService : IEstateManagerServiceInterface, IEstateManagerServiceListAccessInterface
    {
        List<UUI> IEstateManagerServiceListAccessInterface.this[uint estateID]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new SqlCommand("SELECT UserID FROM estate_managers WHERE EstateID = @estateid", conn))
                    {
