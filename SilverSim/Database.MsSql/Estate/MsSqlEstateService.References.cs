...
{
    public sealed partial class MsSqlEstateService : IAssetReferenceInfoServiceInterface
    {
        public void EnumerateUsedAssets(Action<UUID> action)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT DISTINCT CovenantID FROM estates", conn))
                {
