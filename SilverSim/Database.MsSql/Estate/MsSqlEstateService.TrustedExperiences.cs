...
{
    public sealed partial class MsSqlEstateService : IEstateTrustedExperienceServiceInterface
    {
        List<UUID> IEstateTrustedExperienceServiceInterface.this[uint estateID]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new SqlCommand("SELECT ExperienceID FROM estatetrustedexperiences WHERE EstateID = @estateid", conn))
                    {
        ...
        bool IEstateTrustedExperienceServiceInterface.this[uint estateID, UUID experienceID]
        {
            ...
            set
            {
                if (value)
                {
                    ...
                    {
                        conn.Open();
                        conn.ReplaceInto("estatetrustedexperiences", vals, new string[] { "EstateID", "ExperienceID" });
                    }
                }
                else
                {
        ...
        bool IEstateTrustedExperienceServiceInterface.Remove(uint estateID, UUID experienceID)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("DELETE FROM estatetrustedexperiences WHERE EstateID = @estateid AND ExperienceID = @experienceid", conn))
                {
                    cmd.Parameters.AddParameter("@estateid", estateID);
        ...
        bool IEstateTrustedExperienceServiceInterface.TryGetValue(uint estateID, UUID experienceID, out bool trusted)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT NULL FROM estatetrustedexperiences WHERE EstateID = @estateid AND ExperienceID = @experienceid", conn))
                {
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        trusted = reader.Read();
