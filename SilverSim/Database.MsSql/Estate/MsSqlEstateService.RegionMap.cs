...
{
    public sealed partial class MsSqlEstateService : IEstateRegionMapServiceInterface
    {
        List<UUID> IEstateRegionMapServiceInterface.this[uint estateID]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new SqlCommand("SELECT RegionID FROM estate_regionmap WHERE EstateID = @estateid", conn))
                    {
        ...
        bool IEstateRegionMapServiceInterface.Remove(UUID regionID)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("DELETE FROM estate_regionmap WHERE RegionID = @regionid", conn))
                {
                    cmd.Parameters.AddParameter("@regionid", regionID);
        ...
        uint IEstateRegionMapServiceInterface.this[UUID regionID]
        {
            ...
            set
            {
                ...
                {
                    conn.Open();
                    conn.ReplaceInto("estate_regionmap", vals, new string[] { "RegionID" });
                }
            }
        }
    }
