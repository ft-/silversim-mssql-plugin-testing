...
{
    public sealed partial class MsSqlEstateService : IEstateOwnerServiceInterface
    {
        ...
        List<uint> IEstateOwnerServiceInterface.this[UUI owner]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new SqlCommand("SELECT ID, Owner FROM estates WHERE Owner LIKE @agentid", conn))
                    {
        ...
        UUI IEstateOwnerServiceInterface.this[uint estateID]
        {
            ...
            set
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new SqlCommand("UPDATE estates SET Owner = @ownerid WHERE ID = @id", conn))
                    {
                        cmd.Parameters.AddParameter("@id", estateID);
