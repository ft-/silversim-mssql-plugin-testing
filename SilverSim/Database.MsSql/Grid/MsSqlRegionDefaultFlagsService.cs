...
{
    [Description("MsSql RegionDefaultFlags Backend")]
    [PluginName("RegionDefaultFlags")]
    public sealed class MsSqlRegionDefaultFlagsService : RegionDefaultFlagsServiceInterface, IPlugin, IDBServiceInterface
    {
        private readonly string m_ConnectionString;
        private static readonly ILog m_Log = LogManager.GetLogger("MSSQL REGIONDEFAULTFLAGS SERVICE");
        ...
        public override void ChangeRegionDefaultFlags(UUID regionId, RegionFlags addFlags, RegionFlags removeFlags)
        {
            using (var connection = new SqlConnection(m_ConnectionString))
            {
                ...
                connection.InsideTransaction((transaction) =>
                {
                    ...
                    if (haveEntry)
                    {
                        using (var cmd = new SqlCommand("UPDATE regiondefaults SET flags = (flags & @remove) | @add WHERE uuid = @id", connection))
                        {
                            cmd.Transaction = transaction;
                        ...
                        using (var cmd = new SqlCommand("DELETE FROM regiondefaults WHERE flags = 0 AND uuid = @id", connection))
                        {
                            cmd.Transaction = transaction;
                            cmd.Parameters.AddParameter("@id", regionId);
                            cmd.ExecuteNonQuery();
        ...
        public override Dictionary<UUID, RegionFlags> GetAllRegionDefaultFlags()
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("SELECT * FROM regiondefaults", connection))
                {
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
