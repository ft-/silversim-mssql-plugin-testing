...
namespace SilverSim.Database.MsSql.Presence
{
    [Description("MsSql Presence Backend")]
    [PluginName("Presence")]
    public sealed class MsSqlPresenceService : PresenceServiceInterface, IDBServiceInterface, IPlugin, IUserAccountDeleteServiceInterface
    {
        private readonly string m_ConnectionString;
        private static readonly ILog m_Log = LogManager.GetLogger("MSSQL PRESENCE SERVICE");

        #region Constructor
        public MsSqlPresenceService(IConfig ownSection)
        {
        ...
        public override List<PresenceInfo> GetPresencesInRegion(UUID regionId)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT * FROM presence WHERE RegionID = @regionID", conn))
                {
        ...
        public override List<PresenceInfo> this[UUID userID]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new SqlCommand("SELECT * FROM presence WHERE UserID = @userID", conn))
                    {
        ...
        public override void Logout(UUID sessionID, UUID userID)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("DELETE FROM presence WHERE SessionID = @sessionID", conn))
                {
                    cmd.Parameters.AddParameter("@sessionID", sessionID);
        ...
        public override void Report(PresenceInfo pInfo)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("UPDATE presence SET RegionID = @regionID WHERE SessionID = @sessionID", conn))
                {
        ...
        public override void LogoutRegion(UUID regionID)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("DELETE FROM presence WHERE RegionID = @regionid", connection))
                {
                    cmd.Parameters.AddParameter("@regionid", regionID);
                    cmd.ExecuteNonQuery();
                }
        ...
        public override void Remove(UUID scopeID, UUID userAccount)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("DELETE FROM presence WHERE UserID = @userid", conn))
                {
