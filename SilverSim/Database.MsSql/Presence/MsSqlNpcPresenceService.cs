...
{
    [Description("MsSql NpcPresence Backend")]
    [PluginName("NpcPresence")]
    public sealed class MsSqlNpcPresenceService : NpcPresenceServiceInterface, IDBServiceInterface, IPlugin
    {
        private readonly string m_ConnectionString;
        private static readonly ILog m_Log = LogManager.GetLogger("MSSQL PRESENCE SERVICE");

        #region Constructor
        public MsSqlNpcPresenceService(IConfig ownSection)
        {
        ...
        public override void Store(NpcPresenceInfo presenceInfo)
        {
            ...
            using (var conn = new SqlConnection(m_ConnectionString))
            {
                ...
                try
                {
                    conn.ReplaceInto("npcpresence", post, new string[] { "NpcID" });
                }
                catch
                {
        ...
        public override void Remove(UUID scopeID, UUID npcID)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("DELETE FROM npcpresence WHERE NpcID = @userid", conn))
                {
        ...
        private NpcPresenceInfo ReaderToPresenceInfo(SqlDataReader reader) => new NpcPresenceInfo
        {
            Npc = new UUI
            {
        ...
        public override List<NpcPresenceInfo> this[UUID regionID]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new SqlCommand("SELECT * FROM npcpresence WHERE RegionID = @regionID", conn))
                    {
                        cmd.Parameters.AddParameter("@regionID", regionID);
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
