...
namespace SilverSim.Database.MsSql.MuteList
{
    [Description("MsSql MuteList Backend")]
    [PluginName("MuteList")]
    public sealed class MsSqlMuteListService : MuteListServiceInterface, IPlugin, IDBServiceInterface, IUserAccountDeleteServiceInterface
    {
        private static readonly ILog m_Log = LogManager.GetLogger("MSSQL MUTE SERVICE");
        ...
        public override List<MuteListEntry> GetList(UUID muteListOwnerID)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT * FROM mutelists WHERE [agentID] = @agentid", conn))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            res.Add(new MuteListEntry
                            {
                                ...
                                MuteName = (string)reader["muteName"]
                            });
                        }
                    }
                }
        ...
        public void Remove(UUID scopeID, UUID accountID)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("DELETE FROM mutelists WHERE [agentID] = @agentid", conn))
                {
        ...
        public override bool Remove(UUID muteListOwnerID, UUID muteID, string muteName)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("DELETE FROM mutelists WHERE [agentID] = @agentid AND [muteID] = @muteid AND [muteName] = @mutename", conn))
                {
                    cmd.Parameters.AddParameter("@agentid", muteListOwnerID);
        ...
        private static string[] m_Keys = new string[] { "agentID", "muteID", "muteName" };
        public override void Store(UUID muteListOwnerID, MuteListEntry mute)
        {
            using (var conn = new SqlConnection(m_ConnectionString))
            {
                ...
                conn.ReplaceInto("mutelists", vals, m_Keys);
            }
        }
