...
namespace SilverSim.Database.MsSql.UserSession
{
    [Description("MsSql UserSession Backend")]
    [PluginName("UserSession")]
    public sealed class MsSqlUserSessionService : UserSessionServiceInterface, IPlugin, IDBServiceInterface
    {
        private readonly string m_ConnectionString;
        private static readonly ILog m_Log = LogManager.GetLogger("MSSQL USERSESSION SERVICE");

        #region Constructor
        public MsSqlUserSessionService(IConfig ownSection)
        {
        ...
        public override List<UserSessionInfo> this[UGUI user]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new SqlCommand("SELECT * FROM usersessions WHERE [user]=@user", conn))
                    {
        ...
        public override string this[UUID sessionID, string assoc, string varname]
        {
            ...
            set
            {
                using (var conn = new SqlConnection(m_ConnectionString))
                {
                    ...
                    {
                        using (var cmd = new SqlCommand("SELECT NULL FROM usersessions WHERE sessionid = @sessionid", conn)
                        {
        ...
        public override bool ContainsKey(UUID sessionID)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT TOP(1) NULL FROM usersessions WHERE sessionid=@sessionid", conn))
                {
                    cmd.Parameters.AddParameter("@sessionid", sessionID);
                    using (var reader = cmd.ExecuteReader())
                    {
        ...
        public override bool ContainsKey(UGUI user)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT TOP(1) NULL FROM usersessions WHERE [user]=@user", conn))
                {
                    cmd.Parameters.AddParameter("@user", new UGUI(user));
                    using (var reader = cmd.ExecuteReader())
                    {
        ...
        public override bool Remove(UUID sessionID)
        {
            using (var conn = new SqlConnection(m_ConnectionString))
            {
                ...
                {
                    using (var cmd = new SqlCommand("DELETE FROM usersessiondata WHERE sessionid=@sessionid", conn)
                    {
                        Transaction = transaction
                    })
                    {
                    ...
                    using (var cmd = new SqlCommand("DELETE FROM usersessions WHERE sessionid=@sessionid", conn)
                    {
                        Transaction = transaction
                    })
                    {
        ...
        public override bool Remove(UGUI user)
        {
            using (var conn = new SqlConnection(m_ConnectionString))
            {
                ...
                return conn.InsideTransaction((transaction) =>
                {
                    using (var cmd = new SqlCommand("DELETE FROM usersessiondata WHERE sessionid IN (SELECT sessionid FROM usersessions WHERE [user]=@user)", conn)
                    {
                        Transaction = transaction
                    })
                    {
                    ...
                    using (var cmd = new SqlCommand("DELETE FROM usersessions WHERE [user]=@user", conn)
                    {
                        Transaction = transaction
                    })
                    {
        ...
        public override bool CompareAndRemove(UUID sessionID, string assoc, string varname, string value)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("DELETE FROM usersessiondata WHERE sessionid=@sessionid AND assoc=@assoc AND varname=@varname AND [value] = @value AND (isexpiring=0 OR expirydate >= @now)", conn))
                {
                    cmd.Parameters.AddParameter("@sessionid", sessionID);
                    cmd.Parameters.AddParameter("@assoc", assoc);
                    cmd.Parameters.AddParameter("@varname", varname);
        ...
        private void RetrieveData(SqlConnection conn, UserSessionInfo sessionInfo)
        {
            using (var cmd = new SqlCommand("SELECT * FROM usersessiondata WHERE sessionid=@sessionid AND (isexpiring=0 OR expirydate >= @now)", conn))
            {
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        sessionInfo.DynamicData.Add($"{(string)reader["assoc"]}/{(string)reader["varname"]}",
        ...
        public override bool TryGetSecureValue(UUID secureSessionID, out UserSessionInfo sessionInfo)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT * FROM usersessions WHERE securesessionid=@securesessionid", conn))
                {
        ...
        public override bool TryGetValue(UUID sessionID, out UserSessionInfo sessionInfo)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT * FROM usersessions WHERE sessionid=@sessionid", conn))
                {
                    cmd.Parameters.AddParameter("@sessionid", sessionID);
                    using (var reader = cmd.ExecuteReader())
                    {
        ...
        public override bool TryCompareValueExtendLifetime(UUID sessionID, string assoc, string varname, string oldvalue, TimeSpan span, out UserSessionInfo.Entry value)
        {
            ...
            using (var conn = new SqlConnection(m_ConnectionString))
            {
                ...
                success = conn.InsideTransaction((transaction) =>
                {
                    using (var cmd = new SqlCommand("SELECT * FROM usersessiondata WHERE sessionid = @sessionid AND assoc = @assoc AND varname = @varname AND [value] = @value AND (isexpiring=0 OR expirydate >= @now)", conn)
                    {
                        Transaction = transaction
                    })
                    {
                    ...
                    {
                        val.ExpiryDate = val.ExpiryDate.Add(span);
                        using (var cmd = new SqlCommand("UPDATE usersessiondata SET expirydate = @expirydate WHERE sessionid = @sessionid AND assoc = @assoc AND varname = @varname AND [value] = @value", conn)
                        {
                            Transaction = transaction
                        })
                        {
        ...
        public override bool TryCompareAndChangeValueExtendLifetime(UUID sessionID, string assoc, string varname, string oldvalue, string newvalue, TimeSpan span, out UserSessionInfo.Entry value)
        {
            ...
            using (var conn = new SqlConnection(m_ConnectionString))
            {
                ...
                success = conn.InsideTransaction((transaction) =>
                {
                    ...
                    {
                        val.ExpiryDate = val.ExpiryDate.Add(span);
                        using (var cmd = new SqlCommand("UPDATE usersessiondata SET expirydate = @expirydate, [value] = @newvalue WHERE sessionid = @sessionid AND assoc = @assoc AND varname = @varname AND [value] = @value", conn)
                        {
                            Transaction = transaction
                        })
                        {
                    ...
                    {
                        using (var cmd = new SqlCommand("UPDATE usersessiondata SET [value] = @newvalue WHERE sessionid = @sessionid AND assoc = @assoc AND varname = @varname AND [value] = @value", conn)
                        {
                            Transaction = transaction
                        })
                        {
