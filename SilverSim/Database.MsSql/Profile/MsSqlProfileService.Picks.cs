...
namespace SilverSim.Database.MsSql.Profile
{
    public sealed partial class MsSqlProfileService : ProfileServiceInterface.IPicksInterface
    {
        ...
        bool IPicksInterface.ContainsKey(UUI user, UUID id)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT pickuuid FROM userpicks WHERE pickuuid = @uuid", conn))
                {
        ...
        bool IPicksInterface.TryGetValue(UUI user, UUID id, out ProfilePick pick)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT * FROM userpicks WHERE pickuuid = @uuid", conn))
                {
                    cmd.Parameters.AddParameter("@uuid", id);
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
