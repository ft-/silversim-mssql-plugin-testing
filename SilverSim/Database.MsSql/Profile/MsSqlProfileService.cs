...
{
    [Description("MsSql Profile Backend")]
    [PluginName("Profile")]
    public sealed partial class MsSqlProfileService : ProfileServiceInterface, IDBServiceInterface, IPlugin, IUserAccountDeleteServiceInterface
    {
        private static readonly ILog m_Log = LogManager.GetLogger("MSSQL PROFILE SERVICE");

        private readonly string m_ConnectionString;

        public MsSqlProfileService(IConfig ownSection)
        {
        ...
        public override void Remove(UUID scopeID, UUID userAccount)
        {
            using (var conn = new SqlConnection(m_ConnectionString))
            {
                ...
                {
                    using (var cmd = new SqlCommand("DELETE FROM classifieds where creatoruuid = @uuid", conn))
                    {
                        cmd.Transaction = transaction;
                        cmd.Parameters.AddParameter("@uuid", userAccount);
                        cmd.ExecuteNonQuery();
                    }
                    using (var cmd = new SqlCommand("DELETE FROM userpicks where creatoruuid = @uuid", conn))
                    {
                        cmd.Transaction = transaction;
                        cmd.Parameters.AddParameter("@uuid", userAccount);
                        cmd.ExecuteNonQuery();
                    }
                    using (var cmd = new SqlCommand("DELETE FROM usernotes where useruuid = @uuid OR targetuuid = @uuid", conn))
                    {
                        cmd.Transaction = transaction;
                        cmd.Parameters.AddParameter("@uuid", userAccount);
                        cmd.ExecuteNonQuery();
                    }
                    using (var cmd = new SqlCommand("DELETE FROM usersettings where useruuid = @uuid", conn))
                    {
                        cmd.Transaction = transaction;
                        cmd.Parameters.AddParameter("@uuid", userAccount);
                        cmd.ExecuteNonQuery();
                    }
                    using (var cmd = new SqlCommand("DELETE FROM userprofile where useruuid = @uuid", conn))
                    {
                        cmd.Transaction = transaction;
                        cmd.Parameters.AddParameter("@uuid", userAccount);
                        cmd.ExecuteNonQuery();
                    }
                    using (var cmd = new SqlCommand("UPDATE userprofile set profilePartner = '00000000-0000-0000-0000-000000000000' where profilePartner = @uuid", conn))
                    {
                        cmd.Transaction = transaction;
                        cmd.Parameters.AddParameter("@uuid", userAccount);
                        cmd.ExecuteNonQuery();
        ...
        private static readonly IMigrationElement[] Migrations = new IMigrationElement[]
        {
            ...
            new AddColumn<string>("profileWantToText") { Default = string.Empty },
            new AddColumn<uint>("profileSkillsMask") { IsNullAllowed = false },
            new AddColumn<string>("profileSkillsText") { Default = string.Empty },
            new AddColumn<string>("profileLanguages") { Default = string.Empty },
            new AddColumn<UUID>("profileImage") { IsNullAllowed = false, Default = UUID.Zero },
            new AddColumn<string>("profileAboutText") { Default = string.Empty },
            new AddColumn<UUID>("profileFirstImage") { IsNullAllowed = false, Default = UUID.Zero },
            new AddColumn<string>("profileFirstText") { Default = string.Empty },
