...
namespace SilverSim.Database.MsSql.ServerParam
{
    [Description("MsSql ServerParam Backend")]
    [PluginName("ServerParams")]
    public sealed class MsSqlServerParamService : ServerParamServiceInterface, IDBServiceInterface, IPlugin, IPluginShutdown
    {
        private readonly string m_ConnectionString;
        private static readonly ILog m_Log = LogManager.GetLogger("MSSQL SERVER PARAM SERVICE");
        ...
        public void Startup(ConfigurationLoader loader)
        {
            using (var connection = new SqlConnection(m_ConnectionString))
            {
                ...
                using (var cmd = new SqlCommand("SELECT * FROM serverparams", connection))
                {
                ...
                using (var connection = new SqlConnection(m_ConnectionString))
                {
                    ...
                    using (var cmd = new SqlCommand("SELECT * FROM serverparams WHERE parametername = @parametername", connection))
                    {
                        ...
                        using (SqlDataReader dbReader = cmd.ExecuteReader())
                        {
                            ...
                            {
                                UUID regionID = dbReader.GetUUID("regionid");
                                string value = (string)dbReader["parametervalue"];
        ...
        protected override void Store(UUID regionID, string parameter, string value)
        {
            ...
            else
            {
                using (var connection = new SqlConnection(m_ConnectionString))
                {
                    ...
                    connection.ReplaceInto("serverparams", param, new string[] { "regionid", "parameternme" });
                    m_Cache[regionID][parameter] = value;
                }
            }
        }
        ...
        public override List<KeyValuePair<UUID, string>> KnownParameters
        {
            get
            {
                ...
                {
                    connection.Open();
                    using (var cmd = new SqlCommand("SELECT regionid, parametername FROM serverparams", connection))
                    {
                        using (SqlDataReader dbReader = cmd.ExecuteReader())
                        {
                            while (dbReader.Read())
        ...
        public override bool Remove(UUID regionID, string parameter)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("DELETE FROM serverparams WHERE regionid = @regionid AND parametername = @parametername", connection))
                {
                    cmd.Parameters.AddParameter("@regionid", regionID);
                    cmd.Parameters.AddParameter("@parametername", parameter);
