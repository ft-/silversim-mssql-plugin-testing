...
namespace SilverSim.Database.MsSql.Avatar
{
    [Description("MsSql Avatar Backend")]
    [PluginName("Avatar")]
    public sealed class MsSqlAvatarService : AvatarServiceInterface, IDBServiceInterface, IPlugin, IUserAccountDeleteServiceInterface
    {
        private readonly string m_ConnectionString;
        private static readonly ILog m_Log = LogManager.GetLogger("MSSQL AVATAR SERVICE");

        public MsSqlAvatarService(IConfig ownSection)
        {
        ...
        public override Dictionary<string, string> this[UUID avatarID]
        {
            get
            {
                ...
                {
                    connection.Open();
                    using (var cmd = new SqlCommand("SELECT [Name],[Value] FROM avatars WHERE PrincipalID = @principalid", connection))
                    {
            ...
            set
            {
                using (var connection = new SqlConnection(m_ConnectionString))
                {
                    ...
                    if (value == null)
                    {
                        using (var cmd = new SqlCommand("DELETE FROM avatars WHERE PrincipalID = @principalid", connection))
                        {
                            cmd.Parameters.AddParameter("@principalid", avatarID);
                            cmd.ExecuteNonQuery();
                        }
                    ...
                    else
                    {
                        connection.InsideTransaction((transaction) =>
                        {
                            ...
                            foreach (KeyValuePair<string, string> kvp in value)
                            {
                                ...
                                connection.ReplaceInto("avatars", vals, new string[] { "PrincipalID", "Name" }, transaction);
                            }
                        });
                    }
                }
                ...
                using (var connection = new SqlConnection(m_ConnectionString))
                {
                    ...
                    connection.InsideTransaction((transaction) =>
                    {
                        for (int i = 0; i < itemKeys.Count; ++i)
                        {
                            ...
                            vals["Value"] = value[i];
                            connection.ReplaceInto("avatars", vals, new string[] { "Name" }, transaction);
                        }
                    });
                }
            }
        ...
        public override string this[UUID avatarID, string itemKey]
        {
            ...
            set
            {
                using (var connection = new SqlConnection(m_ConnectionString))
                {
                    ...
                    connection.ReplaceInto("avatars", vals, new string[] { "PrincipalID", "Name" });
                }
            }
        }
        ...
        public override void Remove(UUID avatarID, IList<string> nameList)
        {
            using (var connection = new SqlConnection(m_ConnectionString))
            {
                ...
                connection.InsideTransaction((transaction) =>
                {
                    ...
                    {
                        using (var cmd = new SqlCommand("DELETE FROM avatars WHERE PrincipalID = @principalid AND [Name] = @name", connection))
                        {
                            cmd.Transaction = transaction;
