...
namespace SilverSim.Database.MsSql.AuthInfo
{
    [Description("MsSql AuthInfo Backend")]
    [PluginName("AuthInfo")]
    public sealed class MsSqlAuthInfoService : AuthInfoServiceInterface, IDBServiceInterface, IPlugin, IUserAccountDeleteServiceInterface
    {
        private static readonly ILog m_Log = LogManager.GetLogger("MSSQL AUTHINFO SERVICE");
        private readonly string m_ConnectionString;

        public MsSqlAuthInfoService(IConfig ownSection)
        {
        ...
        public void Remove(UUID scopeID, UUID accountID)
        {
            using (var connection = new SqlConnection(m_ConnectionString))
            {
                ...
                {
                    using (var cmd = new SqlCommand("DELETE FROM auth WHERE UserID = @id", connection))
                    {
                        cmd.Transaction = transaction;
                        cmd.Parameters.AddParameter("@id", accountID);
                        cmd.ExecuteNonQuery();
                    }
                    using (var cmd = new SqlCommand("DELETE FROM tokens WHERE UserID = @id", connection))
                    {
                        cmd.Transaction = transaction;
                        cmd.Parameters.AddParameter("@id", accountID);
                        cmd.ExecuteNonQuery();
        ...
        public override void Store(UserAuthInfo info)
        {
            ...
            {
                connection.Open();
                connection.ReplaceInto("auth", vals, new string[] { "UserID" });
            }
        }
        ...
        public override void VerifyToken(UUID principalId, UUID token, int lifetime_extension_in_minutes)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("UPDATE tokens SET Validity=@validity WHERE UserID = @id AND Token = @token AND Validity >= @current", connection))
                {
                ...
                if (!valid)
                {
                    using (var cmd = new SqlCommand("DELETE FROM tokens WHERE Validity <= @current", connection))
                    {
                        cmd.Parameters.AddParameter("@current", Date.Now);
                        cmd.ExecuteNonQuery();
                    }
        ...
        public override void ReleaseToken(UUID accountId, UUID secureSessionId)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("DELETE FROM tokens WHERE UserID = @id AND Token = @token", connection))
                {
                    cmd.Parameters.AddParameter("@id", accountId);
        ...
        public override void ReleaseTokenBySession(UUID accountId, UUID sessionId)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("DELETE FROM tokens WHERE UserID = @id AND SessionID = @sessionid", connection))
                {
                    cmd.Parameters.AddParameter("@id", accountId);
