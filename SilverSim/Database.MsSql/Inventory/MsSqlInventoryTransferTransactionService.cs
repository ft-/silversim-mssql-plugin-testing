...
{
    [Description("MsSql Inventory Transfer Transaction Backend")]
    [PluginName("InventoryTransferTransaction")]
    public sealed class MsSqlInventoryTransferTransactionService : InventoryTransferTransactionServiceInterface, IDBServiceInterface, IPlugin, IUserAccountDeleteServiceInterface
    {
        private readonly string m_ConnectionString;
        private static readonly ILog m_Log = LogManager.GetLogger("MSSQL INVENTORY TRANSFER TRANSACTION SERVICE");
        ...
        public void Remove(UUID userAccount)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("DELETE FROM inventorytransfertransactions WHERE dstagentid LIKE @ownerid", connection))
                {
        ...
        public override bool ContainsKey(UUID userid, UUID dstTransactionID)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("SELECT NULL FROM inventorytransfertransactions WHERE dsttransactionid = @transactionid AND dstagentid LIKE @userid", connection))
                {
        ...
        public override bool TryGetValue(UUID userid, UUID dstTransactionID, out InventoryTransferInfo info)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("SELECT * FROM inventorytransfertransactions WHERE dsttransactionid = @transactionid AND dstagentid LIKE @userid", connection))
                {
                    cmd.Parameters.AddParameter("@transactionid", dstTransactionID);
                    cmd.Parameters.AddParameter("@userid", userid + "%");
                    using (SqlDataReader reader = cmd.ExecuteReader())
        ...
        public override void Store(InventoryTransferInfo info)
        {
            using (var connection = new SqlConnection(m_ConnectionString))
            {
                ...
                connection.ReplaceInto("inventorytransfertransactions", vals, new string[] { "dsttransactionid" });
            }
        }
        ...
        public override bool Remove(UUID userid, UUID dstTransactionID)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("DELETE FROM inventorytransfertransactions WHERE dsttransactionid = @transactionid AND dstagentid LIKE @agentid", connection))
                {
