...
{
    [Description("MsSql Inventory Backend")]
    [PluginName("Inventory")]
    public sealed partial class MsSqlInventoryService : InventoryServiceInterface, IDBServiceInterface, IPlugin, IUserAccountDeleteServiceInterface
    {
        private readonly string m_ConnectionString;
        private static readonly ILog m_Log = LogManager.GetLogger("MSSQL INVENTORY SERVICE");
        ...
        public override List<InventoryItem> GetActiveGestures(UUID principalID)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("SELECT * FROM " + m_InventoryItemTable + " WHERE OwnerID = @ownerid AND AssetType = @assettype AND (flags & 1) <>0", connection))
                {
        ...
        public override List<InventoryFolder> GetInventorySkeleton(UUID principalID)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("SELECT * FROM " + m_InventoryFolderTable + " WHERE OwnerID = @ownerid", connection))
                {
                    cmd.Parameters.AddParameter("@ownerid", principalID);
                    using (SqlDataReader dbReader = cmd.ExecuteReader())
                    {
        ...
        public bool IsParentFolderIdValid(SqlConnection conn, UUID principalID, UUID parentFolderID, UUID expectedFolderID, SqlTransaction transaction = null)
        {
        ...
        public override void Remove(UUID scopeID, UUID userAccount)
        {
            using (var connection = new SqlConnection(m_ConnectionString))
            {
                ...
                {
                    using (var cmd = new SqlCommand("DELETE FROM " + m_InventoryItemTable + " WHERE OwnerID = @ownerid", connection))
                    {
                        cmd.Transaction = transaction;
                        cmd.Parameters.AddParameter("@ownerid", userAccount);
                        cmd.ExecuteNonQuery();
                    }
                    using (var cmd = new SqlCommand("DELETE FROM " + m_InventoryFolderTable + " WHERE OwnerID = @ownerid", connection))
                    {
                        cmd.Transaction = transaction;
                        cmd.Parameters.AddParameter("@ownerid", userAccount);
                        cmd.ExecuteNonQuery();
