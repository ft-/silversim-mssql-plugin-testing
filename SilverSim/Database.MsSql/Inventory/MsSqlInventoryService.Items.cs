...
{
    public sealed partial class MsSqlInventoryService : IInventoryItemServiceInterface
    {
        ...
        void IInventoryItemServiceInterface.Update(InventoryItem item)
        {
            using (var connection = new SqlConnection(m_ConnectionString))
            {
                ...
                connection.InsideTransaction((transaction) =>
                {
                    using (var cmd = new SqlCommand("SELECT TOP(1) AssetID, Flags,AssetType FROM " + m_InventoryItemTable + " WHERE OwnerID = @ownerid AND ID = @itemid", connection)
                    {
        ...
        void IInventoryItemServiceInterface.Delete(UUID principalID, UUID id)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SqlCommand("DELETE FROM " + m_InventoryItemTable + " WHERE OwnerID = @ownerid AND ID = @itemid", connection))
                {
                    cmd.Parameters.AddParameter("@ownerid", principalID);
